...
class IExpress {
    constructor(option) {
        ...
        this.guess_sign = (process) => {
            const lastInfo = process[0];
            for (const template of this.signTemplates) {
                const response = levenshtien(lastInfo, template);
        ...
        this.codeMap = null;
        this.init = () => {
            return this.initCode().pipe(operators_1.tap(codeMap => {
                this.codeMap = codeMap;
                this.codes = new Set(this.codeMap.map(i => i.code));
            }), operators_1.mergeMap(() => (this._init ? this._init() : rxjs_1.of(null)).pipe(operators_1.map(i => this))));
        };
        this.fixCode = (param) => {
